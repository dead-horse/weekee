...
/**
 * init git at directory use gitUrl
 * @param {String} gitUrl      git remote url
 * @param {String} directory   directory path
 */
exports.init = function (gitUrl, directory, callback) {
  ...
  proxy.once('init', function () {
    child_process.exec('git config user.name "Weekee" && git config user.email "node-weekee@"',
  ...
  if (fs.existsSync(directory)) {
    child_process.exec('git init && git remote add origin ' + gitUrl,
      {cwd: directory}, proxy.done('init'));
  } else {
    child_process.exec('git clone ' + gitUrl + ' ' + directory, 
