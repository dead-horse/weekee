...
exports.readFile = function (file, directory, callback) {
  ...
  if (!filePath || !utils.checkFileName(file)) {
...
exports.saveFile = function (file, directory, content, callback) {
  var filePath = utils.getAbsPath(file, directory);
  if (!filePath || !utils.checkFileName(file) || typeof content !== 'string') {
    return process.nextTick(function () {
...
exports.readFolder = function (folderName, directory, callback) {
  ...
  ep.once('readdir', function (fileNames) {
    ...
    fileNames.forEach(function (fileName) {
      ...
      fs.stat(path.join(folderPath, fileName), ep.done(function (stat) {
        if (stat.isDirectory()) {
          files.push({
            name: fileName,
            isDir: true
        ...
        if (stat.isFile()) {
          utils.checkFileName(fileName) && fileName[0] !== '.' && files.push({name: fileName});
          return ep.emit('getFile');
        }
        ep.emit('getFile');
      }));  
