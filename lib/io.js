 ...
 */
function Handler(socket, directory) {
...
/**
 * mv currentDirectory into folderName
 * @param {String} folderName 
 */
Handler.prototype.cdFolder = function(folderName) {
...
Handler.prototype.readFolder = function (input) {
  ...
  var absDirectory = input.directory ? 
    path.join(this.rootDirectory,  input.directory) : this.rootDirectory;
  if (!utils.isSubDir(absDirectory, this.rootDirectory)) {
  ...
  File.readFolder(folderName, absDirectory, function (err, files) {
    ...
    var isRoot = !folderName && utils.isSameDir(absDirectory, self.rootDirectory);
...
exports.create = function (socket, directory) {
  return new Handler(socket, directory);
};
