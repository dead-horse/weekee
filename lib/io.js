 ...
 * @param {Object} markdown   custom markdown parser
 * @param {Boolean} wikiMode  wiki mode will make everything display in markdown
 */
function Handler(socket, directory, git, markdown, wikiMode) {
  this.rootDirectory = path.resolve(directory);
  this.git = git;
  this.socket = socket;
  this.marked = markdown || marked;
...
Handler.prototype.readFile = function (input) {
  ...
  File.readFile(fileName, directory, function (err, data) {
    ...
    var htmlContent = self.wikiMode || utils.isMarkdown(fileName)
    ? self.marked(data || '')
    : self.marked('```\n' + data + '\n```');
...
Handler.prototype.createFile = function (input) {
  ...
  File.createFile(fileName, directory, content, function (err) {
    ...
    return self.socket.emit('createFileReply', {
      originContent: content,
      htmlContent: self.marked(content || ''),
      fileName: fileName
    });
  });
};
...
exports.bind = function (io, directory, git, markdown, wikiMode) {
  io.on('connection', function (socket) {
    new Handler(socket, directory, git, markdown, wikiMode);
  });
};
