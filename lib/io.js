 ...
 */
function Handler(socket, directory, git, markdown) {
...
Handler.prototype.readFile = function (input) {
  ...
  File.readFile(fileName, directory, function (err, data) {
    ...
    socket.emit('readFileReply', {
      originContent: data,
      htmlContent: utils.isMarkdown(fileName) ? self.marked(data || '') : self.marked('```\n' + data + '\n```'),
      fileName: fileName
    });
  });
};
...
Handler.prototype.saveFile = function(input) {
  ...
  File.saveFile(fileName, directory, content, function (err) {
    ...
    return self.socket.emit('saveFileReply', {
      ...
      htmlContent: utils.isMarkdown(fileName) ? self.marked(content || '') : self.marked('```\n' + (content || '') + '\n```'),
      fileName: fileName      
    });
  });
};
...
exports.bind = function (io, directory, git, markdown) {
  io.on('connection', function (socket) {
    new Handler(socket, directory, git, markdown);
  });
};
